...
use gnome_panel_core::config::{WidgetEntry, WidgetOrGroup};
use gnome_panel_core::{Config, ThemePalette};
...
pub fn create_bar_window(
    ...
    window.set_namespace(Some("gnome-panel"));
...
/// 2. `$XDG_CONFIG_HOME/gnome-panel/style.css`
/// 3. `~/.config/gnome-panel/style.css`
...
fn user_css_search_paths_from_env(
    ...
    if let Some(xdg_config) = xdg_config_home {
        push_unique(xdg_config.join("gnome-panel/style.css"));
    }

    // 3. ~/.config/gnome-panel/style.css
    if let Some(home) = home_dir {
        push_unique(home.join(".config/gnome-panel/style.css"));
    }
...
mod tests {
    ...
    fn user_css_search_paths_config_dir_first() {
        ...
        assert_eq!(
            ...
            vec![
                dir.join("style.css"),
                PathBuf::from("/xdg-home/gnome-panel/style.css"),
                PathBuf::from("/home/test/.config/gnome-panel/style.css"),
    ...
    fn user_css_search_paths_deduplicates() {
        let paths = user_css_search_paths_from_env(
            Some(Path::new("/home/test/.config/gnome-panel")),
    ...
    fn find_user_css_returns_none_when_no_file_exists() {
        ...
        let tmp = TestDir::new("gnome-panel_test_missing_css");
    ...
    fn find_user_css_finds_existing_file() {
        ...
        let tmp = TestDir::new("gnome-panel_test_css");
