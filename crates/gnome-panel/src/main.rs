...
use gnome_panel_core::{Config, logging};
...
fn main() -> ExitCode {
    ...
    if args.print_example_config {
        print!("{}", gnome_panel_core::config::DEFAULT_CONFIG_TOML);
        return ExitCode::SUCCESS;
    }
    ...
    match action {
        InhibitAction::Toggle => {
            ...
            match send_ipc_message(&msg) {
                ...
                Err(e) => {
                    eprintln!(
                        "Error: could not reach gnome-panel IPC socket (is the panel running?): {}",
                        e
                    );
                    ExitCode::FAILURE
                }
...
fn run_gtk_app(config: Config, config_source: Option<PathBuf>) -> ExitCode {
    ...
    app.connect_activate(move |app| {
        ...
        // `gnome-panel inhibit` and other CLI commands always work.
        {
            ...
            if let Some(listener) = IpcListener::new() {
                ...
                // SAFETY: Key is unique to gnome-panel; stored type matches what
                // would be retrieved. The data keeps the Rc alive for the
                // application's lifetime.
                unsafe {
                    app.set_data("gnome-panel-ipc-listener", listener);
                }
            ...
            if let Some(overlay) = osd_overlay {
                ...
                // application's lifetime.
                unsafe {
                    app.set_data("gnome-panel-osd-overlay", overlay);
                }
