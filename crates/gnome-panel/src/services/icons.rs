//! IconsService - process-wide icon management for the gnome-panel bar.
//!
...
"#,
    ...
    ///
    /// The font is written to `$XDG_CACHE_HOME/gnome-panel/fonts/MaterialSymbolsRounded.ttf`
    /// (or `~/.cache/gnome-panel/fonts/` if XDG_CACHE_HOME is not set).
    ...
    fn extract_embedded_font() -> Option<PathBuf> {
        ...
        let font_dir = cache_dir.join("gnome-panel").join("fonts");
