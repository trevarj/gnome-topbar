...
fn pulse_worker_thread(command_rx: Receiver<AudioCommand>) {
    ...
    proplist
        .set_str(pulse::proplist::properties::APPLICATION_NAME, "gnome-panel")
        .ok();
    proplist
        .set_str(
    ...
    let context = match Context::new_with_proplist(&mainloop, "gnome-panel-audio", &proplist) {
        Some(ctx) => ctx,
        None => {
...
impl AudioCli {
    ...
    pub fn new(user_max_percent: u32) -> Option<Self> {
        ...
        let mut context = Context::new_with_proplist(&mainloop, "gnome-panel-cli", &proplist)?;
