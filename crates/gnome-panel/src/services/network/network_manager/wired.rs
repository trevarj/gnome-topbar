...
impl NmService {
    ...
    pub(super) fn fetch_wired_device_info() {
        thread::spawn(move || {
            ...
            if std::path::Path::new("/tmp/gnome-panel-debug-wired").exists() {
...
///
/// In debug builds, overridable via `/tmp/gnome-panel-debug-wired`:
/// - Enable: `touch /tmp/gnome-panel-debug-wired`
/// - Disable: `rm /tmp/gnome-panel-debug-wired`
