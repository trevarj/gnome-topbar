...
use gnome_panel_core::theme::{BORDER_OPACITY_DARK, BORDER_OPACITY_GTK, BORDER_OPACITY_LIGHT};
use gnome_panel_core::{Config, ThemePalette, ThemeSizes};
...
impl ConfigManager {
    ...
    ///
    /// When true, gnome-panel sends ext-background-effect-v1 blur region hints
...
fn widget_names(config: &Config) -> Vec<String> {
    use gnome_panel_core::config::WidgetPlacement;
...
mod tests {
    ...
    fn test_make_absolute_passthrough_for_absolute_path() {
        let absolute = Path::new("/tmp/gnome-panel-style.css");
    ...
    fn test_widget_names_includes_show_if_fields() {
        use gnome_panel_core::config::{WidgetOptions, WidgetPlacement};
