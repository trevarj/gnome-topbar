...
//!
//! Uses a Unix datagram socket in `$XDG_RUNTIME_DIR/gnome-panel.sock`.
...
///
/// Returns `$XDG_RUNTIME_DIR/gnome-panel.sock` or falls back to `/tmp/gnome-panel.sock`.
pub fn socket_path() -> PathBuf {
    if let Ok(runtime_dir) = std::env::var("XDG_RUNTIME_DIR") {
        PathBuf::from(runtime_dir).join("gnome-panel.sock")
    } else {
        warn!("XDG_RUNTIME_DIR not set, falling back to /tmp/gnome-panel.sock");
        PathBuf::from("/tmp/gnome-panel.sock")
    }
}
