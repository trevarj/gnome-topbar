...
//! CLI commands (`gnome-panel media play-pause`, etc.) can control the same
//! player that's shown in the UI.
//!
//! State file: `$XDG_RUNTIME_DIR/gnome-panel-media-state` contains:
...
fn state_file_path() -> PathBuf {
    if let Ok(runtime_dir) = std::env::var("XDG_RUNTIME_DIR") {
        PathBuf::from(runtime_dir).join("gnome-panel-media-state")
    } else {
        PathBuf::from("/tmp/gnome-panel-media-state")
    }
}
