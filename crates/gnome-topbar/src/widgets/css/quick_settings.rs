...
pub fn css(animations: bool) -> String {
    // The expander chevron / row-menu icon rotation (when a row expands) is now
    // frame-clock driven from Rust via a `RotateBox` snapshot wrapper around the
    // icon (see widgets/rotate_box.rs and quick_settings/ui_helpers.rs). The old
    // CSS `transition: transform 225ms ease` + `transform: rotate(...)` on the
    // `.expanded` class has been removed; the Rust animation honors
    // `theme.animations`. `animations` is no longer read here.
    let _ = animations;
    r#"
/* ===== QUICK SETTINGS ===== */
...
/* Row-menu icon rotation is Rust-driven (RotateBox); no CSS transform. */
...
/* Chevron icon. Rotation on expand is Rust-driven (RotateBox); no CSS
   transform. The margin shift between states is kept as a class toggle. */
.qs-toggle-more-icon {
...
mod tests {
    ...
    #[test]
    fn quick_settings_css_has_no_transform_transitions() {
        // Chevron / row-menu rotation is now Rust-driven (RotateBox); the CSS
        // must not emit transform transitions or rotate() rules in either mode.
        for css in [css(true), css(false)] {
            assert!(!css.contains("transition: transform"));
            assert!(!css.contains("transform: rotate"));
        }
    }
}
