...
//! CLI commands (`gnome-topbar media play-pause`, etc.) can control the same
//! player that's shown in the UI.
//!
//! State file: `$XDG_RUNTIME_DIR/gnome-topbar-media-state` contains:
...
fn state_file_path() -> PathBuf {
    if let Ok(runtime_dir) = std::env::var("XDG_RUNTIME_DIR") {
        PathBuf::from(runtime_dir).join("gnome-topbar-media-state")
    } else {
        PathBuf::from("/tmp/gnome-topbar-media-state")
    }
}
