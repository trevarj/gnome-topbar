...
pub struct AudioService {
    ...
    /// Maximum percentage GNOME Topbar is allowed to request from user-facing controls.
...
fn pulse_worker_thread(command_rx: Receiver<AudioCommand>) {
    ...
    proplist
        .set_str(
            ...
            "io.github.trevarj.GnomePanel",
        )
        .ok();

    let context = match Context::new_with_proplist(&mainloop, "gnome-topbar-audio", &proplist) {
        Some(ctx) => ctx,
        None => {
...
pub struct AudioCli {
    ...
    control_available: bool,
    /// Maximum percentage GNOME Topbar CLI commands are allowed to request.
...
impl AudioCli {
    ...
    pub fn new(user_max_percent: u32) -> Option<Self> {
        ...
        let mut context = Context::new_with_proplist(&mainloop, "gnome-topbar-cli", &proplist)?;
