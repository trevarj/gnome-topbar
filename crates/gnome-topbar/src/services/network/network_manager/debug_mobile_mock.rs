...
//!
//! **Opt-in**: set `GNOME_TOPBAR_MOCK_MOBILE=1` in the environment and create
//! `/tmp/gnome-topbar-debug-mobile` with a state string. The panel polls the file
...
//!
//! ```sh
//! export GNOME_TOPBAR_MOCK_MOBILE=1                        # enable mock (once per session)
//! echo "connected" > /tmp/gnome-topbar-debug-mobile    # simulate connected modem
//! echo "disabled"  > /tmp/gnome-topbar-debug-mobile     # simulate disabled modem
//! rm /tmp/gnome-topbar-debug-mobile                     # back to normal
//! ```
...
/// 1. The `GNOME_TOPBAR_MOCK_MOBILE=1` environment variable is set.
/// 2. The debug mock file (`/tmp/gnome-topbar-debug-mobile`) exists.
///
...
pub fn is_enabled() -> bool {
    std::env::var("GNOME_TOPBAR_MOCK_MOBILE")
        .map(|v| v == "1")
        .unwrap_or(false)
