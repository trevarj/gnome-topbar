...
//!
//! Uses a Unix datagram socket in `$XDG_RUNTIME_DIR/gnome-topbar.sock`.
...
///
/// Returns `$XDG_RUNTIME_DIR/gnome-topbar.sock` or falls back to `/tmp/gnome-topbar.sock`.
pub fn socket_path() -> PathBuf {
    if let Ok(runtime_dir) = std::env::var("XDG_RUNTIME_DIR") {
        PathBuf::from(runtime_dir).join("gnome-topbar.sock")
    } else {
        warn!("XDG_RUNTIME_DIR not set, falling back to /tmp/gnome-topbar.sock");
        PathBuf::from("/tmp/gnome-topbar.sock")
    }
}
