//! gnome-topbar - GNOME Shell-inspired GTK top panel for Wayland
//!
//! This is the main entry point for the gnome-topbar bar application.
...
use gnome_topbar_core::{Config, logging};
...
fn main() -> ExitCode {
    ...
    if args.print_example_config {
        print!("{}", gnome_topbar_core::config::DEFAULT_CONFIG_TOML);
        return ExitCode::SUCCESS;
    }
...
fn run_gtk_app(config: Config, config_source: Option<PathBuf>) -> ExitCode {
    ...
    let app = Application::builder()
        .application_id("io.github.trevarj.GnomePanel")
    ...
    app.connect_activate(move |app| {
        ...
        // `gnome-topbar inhibit` and other CLI commands always work.
        {
            ...
            if let Some(listener) = IpcListener::new() {
                ...
                // SAFETY: Key is unique to gnome-topbar; stored type matches what
                // would be retrieved. The data keeps the Rc alive for the
                // application's lifetime.
                unsafe {
                    app.set_data("gnome-topbar-ipc-listener", listener);
                }
            ...
            if let Some(overlay) = osd_overlay {
                ...
                // application's lifetime.
                unsafe {
                    app.set_data("gnome-topbar-osd-overlay", overlay);
                }
