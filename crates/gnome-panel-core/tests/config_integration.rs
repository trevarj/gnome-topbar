...
use gnome_panel_core::Config;
use std::path::PathBuf;

fn project_root() -> PathBuf {
    // Navigate from crates/gnome-panel-core/ up to project root
...
fn test_widget_names() {
    ...
    let right_names: Vec<String> = config
        ...
        .flat_map(|w| w.display_names())
        .collect();
    let center_names: Vec<String> = config
        .widgets
        .center
        .iter()
        .flat_map(|w| w.display_names())
    ...
    assert!(
        center_names.iter().any(|n| n.contains("clock")),
...
fn test_broken_config_returns_error_not_defaults() {
    ...
    let temp_dir = std::env::temp_dir().join("gnome-panel_test_broken_config");
...
fn test_audio_overdrive_policy_explicit_true() {
    let temp_dir = std::env::temp_dir().join("gnome-panel_test_audio_policy_true");
...
fn test_audio_overdrive_policy_explicit_false() {
    let temp_dir = std::env::temp_dir().join("gnome-panel_test_audio_policy_false");
    let _ = std::fs::remove_dir_all(&temp_dir);
    std::fs::create_dir_all(&temp_dir).unwrap();
...
fn test_audio_overdrive_policy_missing_or_invalid_falls_back() {
    let temp_dir = std::env::temp_dir().join("gnome-panel_test_audio_policy_invalid");
    let _ = std::fs::remove_dir_all(&temp_dir);
    std::fs::create_dir_all(&temp_dir).unwrap();
...
fn test_audio_overdrive_policy_ignores_unrelated_validation_errors() {
    let temp_dir = std::env::temp_dir().join("gnome-panel_test_audio_policy_lenient");
    let _ = std::fs::remove_dir_all(&temp_dir);
    std::fs::create_dir_all(&temp_dir).unwrap();
