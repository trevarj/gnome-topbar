//! Unified theming system for gnome-topbar.
//!
...
const DEFAULT_FONT_FAMILY: &str = "Adwaita Sans, Cantarell, Noto Sans, sans-serif";
...
mod tests {
    ...
    fn test_accent_default_is_custom() {
        // GNOME Topbar's shipped default uses GNOME blue as the custom accent.
        let mut config = Config::default();
        config.theme.mode = "dark".to_string();
        let palette = ThemePalette::from_config(&config, None, None);
