...
fn project_root() -> PathBuf {
    // Navigate from crates/gnome-topbar-core/ up to project root
...
fn test_broken_config_returns_error_not_defaults() {
    ...
    let temp_dir = std::env::temp_dir().join("gnome-topbar_test_broken_config");
...
fn test_audio_overdrive_policy_explicit_true() {
    let temp_dir = std::env::temp_dir().join("gnome-topbar_test_audio_policy_true");
...
fn test_audio_overdrive_policy_explicit_false() {
    let temp_dir = std::env::temp_dir().join("gnome-topbar_test_audio_policy_false");
    let _ = std::fs::remove_dir_all(&temp_dir);
    std::fs::create_dir_all(&temp_dir).unwrap();
...
fn test_audio_overdrive_policy_missing_or_invalid_falls_back() {
    let temp_dir = std::env::temp_dir().join("gnome-topbar_test_audio_policy_invalid");
    let _ = std::fs::remove_dir_all(&temp_dir);
    std::fs::create_dir_all(&temp_dir).unwrap();
...
fn test_audio_overdrive_policy_ignores_unrelated_validation_errors() {
    let temp_dir = std::env::temp_dir().join("gnome-topbar_test_audio_policy_lenient");
    let _ = std::fs::remove_dir_all(&temp_dir);
    std::fs::create_dir_all(&temp_dir).unwrap();
